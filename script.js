...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTotals() {
        ...
        dataRows.forEach(data => {
            ...
            let amountInIQD = currency === '(USD)' ? amount * exchangeRate : amount;

            if (type.includes('Expense')) {
                expenses += amountInIQD;
                total -= amountInIQD;
        ...
        if (index > 0) { // Skip header row
            ...
            tags.forEach(tag => {
                ...
                if (cells[1].textContent.includes('Income')) {
                    data.tags[tag].income += amountInIQD;
                } else if (cells[1].textContent.includes('Expense')) {
                    data.tags[tag].expenses += amountInIQD;
                }
            });
...
// Move loadData function outside of DOMContentLoaded
function loadData() {
